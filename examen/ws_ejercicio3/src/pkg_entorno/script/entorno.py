...
def main():
    rospy.init_node("entorno_crb15000", anonymous=True)    
    ...
    aux_pose.pose.position.z = 1.0 - dist_kuka_piso
    box_name = "wall_back"
    scene.add_box(box_name, aux_pose, size=(0.1, 2.1, 2.0))  

    aux_pose = geometry_msgs.msg.PoseStamped()
    aux_pose.header.frame_id = "base_link"
    aux_pose.pose.orientation.w = 1.0
    aux_pose.pose.position.x = 0.5
    aux_pose.pose.position.y = -0.4
    aux_pose.pose.position.z = 0.5
    box_name = "pilar"
    scene.add_box(box_name, aux_pose, size=(0.5, 0.5, 0.7))  
